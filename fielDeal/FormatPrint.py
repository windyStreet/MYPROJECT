...
def writeLogFile(logStr,type):
    ...
    typeSize = os.path.getsize(typeLogPath)
    if (typeSize/1024/1024) > 30:
        new_file = sys.path[0] + os.sep + 'logs' + os.sep + time.strftime('%Y-%m-%d-%H-%M-%S') + "-"+typeLogPath.split(os.sep)[-1]
        os.rename(typeLogPath,new_file)

    size = os.path.getsize(logPath)
    if (size/1024/1024) > 30:
        new_file_1 = sys.path[0] + os.sep + 'logs' + os.sep + time.strftime('%Y-%m-%d-%H-%M-%S') + "-"+logPath.split(os.sep)[-1]
        os.rename(logPath, new_file_1)

...
def writeLogFile(logStr, type):
    ...
    if type == 'info':
        systematicPath = sys.path[0] + os.sep + 'logs' + os.sep + 'info.log'
    elif type == 'err':
        systematicPath = sys.path[0] + os.sep + 'logs' + os.sep + 'error.log'
    elif type == 'war':
        systematicPath = sys.path[0] + os.sep + 'logs' + os.sep + 'warning.log'
    elif type == 'debug':
        systematicPath = sys.path[0] + os.sep + 'logs' + os.sep + 'debug.log'
    elif type == 'falat':
        systematicPath = sys.path[0] + os.sep + 'logs' + os.sep + 'falat.log'
    else:
        systematicPath = sys.path[0] + os.sep + 'logs' + os.sep + 'log.log'
    ...
    if (systematicSize / 1024 / 1024) > 30:
        systematic_file = sys.path[0] + os.sep + 'logs' + os.sep + time.strftime('%Y-%m-%d-%H-%M-%S') + "-" + \
                          systematicPath.split(os.sep)[-1]
        os.rename(systematicPath, systematic_file)

    logSize = os.path.getsize(logPath)
    if (logSize / 1024 / 1024) > 30:
        log_file = sys.path[0] + os.sep + 'logs' + os.sep + time.strftime('%Y-%m-%d-%H-%M-%S') + "-" + \
                   logPath.split(os.sep)[-1]

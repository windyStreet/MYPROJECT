...
def createNeedModifyDataFiles(tablenames):
    ...
    for tablename in tablenames:
        ...
        for pagenum in range(0,pageCount):
            ...
            conditon = " nickname = '李云红' limit " + limt + " offset " + offset
...
def dealNeedModifyData():
    ...
    for fileName in files:
        ...
        for eachLine in fopen:
            ...
            jo = json.loads(eachLine,encoding='utf-8')
            returnfields = ['id', 'cardno','nickname']
            returnFieldsStr = ","
            returnFieldsStr = returnFieldsStr.join(returnfields)
            conditon = " id = '"+jo['id']+"' ;"
    ...
    for fileName in files:
        ...
        for eachLine in fopen:
            try:
                eachLine = eachLine.replace("\n", "")
                jo = json.loads(eachLine,encoding='utf-8')
                if  jo['nickname']  == None or jo['nickname'] == "None":
                    nickname = "null"
                else:
                    nickname = "'"+str(jo['nickname'])+"'"

                    UpdataSQL = "update " + tableName + " set nickname = " + nickname + " where id = '" + jo['id'] + "' returning id ;"
...
def dealToJSONStr(fields,rows):
    ...
    for row in rows:
        ...
        for index in range(0, len(fields)):
            ...
            needExportStr = json.dumps(toDealData, ensure_ascii=False)+"\n"
